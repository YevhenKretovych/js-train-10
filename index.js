...
function customAssign(...objects) {
  ...
  if ([...objects].length < 2) {
    ...
    let objAssign = Object.assign(...objects);
    let objectNew = {};

    for (const key in objAssign) {
      let item = objAssign[key];
      if (typeof item === "number") {
        objectNew[key] = item.toString();
      } else {
        objectNew[key] = item;
      }
    }
...
function customObjectFromEntries(entries) {
  ...
  if (Array.isArray(entries) === false) {
    ...
    let arrMap = entries.map((element) => {
      let key = element[0];
      let value = element[1];
...
function keysAndValues(obj) {
  ...
  if (typeof obj !== "object") {
    ...
    //   let arrays = Object.entries(objKey, objValue);
    let arrays = [[...objKey], [...objValue]];
...
function filterObjectsByKey(arr, key) {
  ...
  if (Array.isArray(arr) !== true) {
    ...
    const newArr = arr.filter((element) => element[key]);
...
function checkAndPreventExtensions(obj) {
  ...
  if (typeof obj !== "object") {
    return false;
  } else if (Object.isExtensible(obj) === true) {
...
function getObjectValuesSum(obj) {
  ...
  if (typeof obj !== "object") {
    ...
    let sumValue = objValue.reduce((num, element) => num + element, 0);
